...
export default async function ProfilePage({ params }: ProfilePageProps) {
  ...
  const typeIcons: Record<string, string> = {
    photo: "📸", project: "🚀", career: "💼", idea: "💡", article: "✍️",
  };
    ...
    <main className="max-w-[1100px] mx-auto px-8 py-12 relative">
      ...
      <section className="mb-14">
        ...
        <div className="mx-auto" style={{ maxWidth: 680 }}>
          {whatsNew.length > 0 ? (
            <WhatsNewFeed initialItems={whatsNew} username={username} />
          ) : legacyFeed ? (
            /* Fallback: legacy buildFeed 3-column grid */
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {legacyFeed.map((item, i) => (
                ...
                >
                  ...
                  {item.image && (
                    // eslint-disable-next-line @next/next/no-img-element
                    <img src={item.image} alt={item.title} className="w-full h-28 object-cover border-b-3 border-ink" />
                  )}
                  <div className="p-5 pt-4 relative">
                    ...
                    <p className="text-[0.82rem] opacity-70 leading-snug">{item.desc}</p>
                  </div>
                </Link>
              ))}
            </div>
          ) : (
            <p className="py-8 text-center font-mono text-[0.85rem]" style={{ opacity: 0.4 }}>
